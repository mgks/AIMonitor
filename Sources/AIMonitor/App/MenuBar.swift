...
enum MonitorMenuBarIcon {
    /// Template image rasterised from the SVG glyph at 2x for crisp edges.
    static let image: NSImage = {
        ...
        """
        let temp = "/tmp/aimonitor-menubar.svg"
        ...
        guard let svgImage = NSImage(contentsOfFile: temp) else {
            ...
            fallback.lockFocus()
            NSBezierPath(ovalIn: NSRect(x: 3, y: 3, width: 9, height: 9)).stroke()
            fallback.unlockFocus()
            fallback.isTemplate = true
            return fallback
        }
        // Render at 2x for retina crispness, display at 15pt (between 13 and 18).
        let renderSize: CGFloat = 30
